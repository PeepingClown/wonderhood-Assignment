...
(function($) {
  ...
  Annotate.prototype = {
    ...
    resizeCropSelect: function (event) {
        ...
        if (n_x + n_w > $canvas.width() ) { n_x = event.data.old.x - ($canvas.width() - (event.data.old.x+event.data.old.w));
                                            n_w = event.data.old.w + 2*($canvas.width() - (event.data.old.x+event.data.old.w));}
        if (n_y + n_h > $canvas.width() ) { n_y = event.data.old.y - ($canvas.height() - (event.data.old.y+event.data.old.h));
                                            n_h = event.data.old.h + 2*($canvas.height() - (event.data.old.y+event.data.old.h));}
        /*MIN SIZE*/
        if (n_w < self.cropSizeMinW) {n_w = self.cropSizeMinW;
                        n_x = Math.floor((event.data.old.w-n_w)/2) + event.data.old.x};
        if (n_h < self.cropSizeMinH) {n_h = self.cropSizeMinH;
    ...
    moveModeMouseDown: function (event) {
        ...
        if (clickedElement) {
            ...
            $canvas.bind("mousemove", {canvas: $canvas, self:self, 
                element:newElement, 
        ...
        for (var i = self.storedElement.length-1; i>=0; i--) {
            ...
            if (x_min <= rel_x &&
                ...
                return {element: element,       // The Element
                        ...
                        rel:{x:rel_x, y:rel_y}};// Position of the Cursor
            }
        }
    },
    ...
    redraw: function() {
      ...
      for (var i = 0; i < self.storedElement.length; i++) {
          ...
          switch (element.type) {
            case 'mirror':
                if (element.orientation == 'vertical') {mirror_v = -1*mirror_v;}
                else if (element.orientation == 'horizontal') {mirror_h = -1*mirror_h;}
                break;
            case 'rotate':
    ...
    getTextBounds: function(drawingContext, element) {
        ...
        return {w: (width>element.maxwidth?element.maxwidth+14:width+14),
